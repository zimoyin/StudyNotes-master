...
文章来源:[**Nyima** ](https://nyimac.gitee.io/2021/04/18/Netty%E5%AD%A6%E4%B9%A0%E4%B9%8BNIO%E5%9F%BA%E7%A1%80/#Netty%E5%AD%A6%E4%B9%A0%E4%B9%8BNIO%E5%9F%BA%E7%A1%80)
...
**NIO**（Non-blocking I/O，在Java领域，也称为New I/O），是一种**同步非阻塞的I/O模型**，~~也是I/O多路复用的基础~~，已经被越来越多地应用到大型应用服务器，成为解决高并发与大量连接、I/O处理问题的有效方式。

**NIO**支持**面向缓冲区的、基于通道的IO操作**。NIO将以更加高效的方式进行文件的读写操作。



##### 2. NIO和BIO 以及区别

NIO (Non-blocking Input/Output) 和 BIO (Blocking Input/Output) 是两种不同的网络编程模型，用于实现客户端和服务器之间的通信。

**BIO** 模型是阻塞式的，意味着当一个线程调用 read 方法时，它会阻塞，直到有数据可读为止。这意味着一个线程只能处理一个客户端的请求，因此在高并发的情况下，BIO 模型的性能较差。

**相比之下，NIO** 模型是非阻塞式的，意味着当一个线程调用 read 方法时，它不会阻塞，而是立即返回。如果没有数据可读，read 方法会返回 -1。在 NIO 模型中，一个线程可以处理多个客户端的请求，因此在高并发的情况下，NIO 模型的性能较好。

另外，NIO 模型还支持事件驱动，意味着它可以在数据可读或可写时通知应用程序，而不是轮询数据状态。这使得 NIO 模型更加高效，因为它不需要浪费时间去轮询数据状态。
...
[![img](nio.assets/20210412135510.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210412135510.png)
...
[![img](nio.assets/20210418181918.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210418181918.png)
...
[![img](nio.assets/20210418181933.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210418181933.png)
...
[![img](nio.assets/20210418181947.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210418181947.png)
...
[![img](nio.assets/20210412155726.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210412155726.png)
...
[![img](nio.assets/20210413213318.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210413213318.png)

- 客户端-服务器建立连接后，客户端发送消息前：服务器端因通道为空被阻塞

[![img](nio.assets/20210413213446.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210413213446.png)

- 客户端发送数据后，服务器处理通道中的数据。再次进入循环时，再次被accept阻塞

[![img](nio.assets/20210413214109.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210413214109.png)

- 之前的客户端再次发送消息**，服务器端因为被accept阻塞**，无法处理之前客户端发送到通道中的信息

[![img](nio.assets/20210413214505.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210413214505.png)
...
- 当调用了 server.register(selector, SelectionKey.OP_ACCEPT)后，Selector中维护了一个集合，**用于存放SelectionKey以及其对应的通道**
  ...
  [![img](nio.assets/20210414192429.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210414192429.png)

- 当**选择器中的通道对应的事件发生后**，selecionKey会被放到另一个集合中，但是**selecionKey不会自动移除**，所以需要我们在处理完一个事件后，通过迭代器手动移除其中的selecionKey。否则会导致已被处理过的事件再次被处理，就会引发错误[![img](nio.assets/20210414193143.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210414193143.png)
...
[![img](nio.assets/20210415103442.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210415103442.png)
...
- TLV 格式，即 Type 类型、Length 长度、Value 数据
  ...
  - Http 1.1 是 TLV 格式
  - Http 2.0 是 LTV 格式[![img](nio.assets/20210415103926.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210415103926.png)
...
- 复制数据阶段
  ...
  [![img](nio.assets/20210418151243.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210418151243.png)

根据UNIX 网络编程 - 卷 I，IO模型主要有以下几种

### 阻塞IO

[![img](nio.assets/20210418151605.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210418151605.png)

- 用户线程进行read操作时，**需要等待操作系统执行实际的read操作**，此期间用户线程是被阻塞的，无法执行其他操作

### 非阻塞IO

[![img](nio.assets/20210418152137.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210418152137.png)
...
[![img](nio.assets/20210418154208.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210418154208.png)
...
[![img](nio.assets/20210418160106.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210418160106.png)
...
[![img](nio.assets/20210418162306.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210418162306.png)
...
[![img](nio.assets/20210418162410.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210418162410.png)
...
[![img](nio.assets/20210418162750.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210418162750.png)
...
[![img](nio.assets/20210418163033.png)](https://nyimapicture.oss-cn-beijing.aliyuncs.com/img/20210418163033.png)
